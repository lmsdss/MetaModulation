...
class DermNet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.args.train:
            ...
            self.choose_classes = np.random.choice(self.classes_idx, size=self.nb_classes, replace=False)
            query_size_test = sum(
                [self.data[self.choose_classes[j]].shape[0] for j in range(self.nb_classes)]) - self.set_size
...
class MiniImagenet(Dataset):
    ...
    def __init__(self, args, mode):
        ...
        self.k_shot = args.update_batch_size  # k-shot  1 / 5
        self.k_query = args.update_batch_size_eval  # for evaluation  15
        self.set_size = self.n_way * self.k_shot  # num of samples per set  # 5
        self.query_size = self.n_way * self.k_query  # number of samples per set for evaluation # 75
        self.mode = mode
        if mode == 'train':
        ...
        self.num_train_use_class = int(64 * self.args.ratio) #  64*0.2
    ...
    def __getitem__(self, index):
        if self.mode == 'train':  # diff
            self.classes_idx = self.all_train_classes[:self.num_train_use_class]  # diff
        else:
